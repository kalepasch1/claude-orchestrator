...
describe('buildPreservationPlan', () => {
  ...
  it('skips anything that is not recoverable, or has no sha, or has no ref to push', () => {
    const { atRisk } = buildPreservationPlan([
      branch('agent/present', { classification: 'ALREADY_PRESENT' }),
      branch('agent/no-sha', { sha: null }),
      { ...branch('agent/wt'), kind: 'worktree', ref: '/some/path' },
      { ...branch('agent/stash'), kind: 'stash', ref: 'stash@{0}' },
    ])
    assert.equal(atRisk, 0)
  })

  it('preserves rescue and archive refs, not just branches', () => {
    // This previously asserted the opposite, and that was the bug. On the real
    // repository the run reported "518 items have NO remote copy — the only copy is on
    // this disk" and then emitted a plan covering 11 of them, because 507 were
    // rescue/archive refs and the filter was `kind === 'branch'`. A sweep ref is the
    // most at-risk evidence there is — it exists precisely because the work never got
    // a branch — so excluding it inverted the tool's own warning.
    const { script, atRisk } = buildPreservationPlan([
      { ...branch('x'), kind: 'rescue_ref', ref: 'refs/orch-rescue/20260803T0007-x' },
      { ...branch('y'), kind: 'archive_ref', ref: 'refs/archive/agent/y/1785984644' },
      { ...branch('z'), kind: 'quarantine_ref', ref: 'refs/quarantine/z' },
    ])
    assert.equal(atRisk, 3)
    assert.match(script, /push a{40} refs\/preserved\/orch-rescue\/20260803T0007-x/)
    assert.match(script, /push a{40} refs\/preserved\/archive\/agent\/y\/1785984644/)
    assert.match(script, /push a{40} refs\/preserved\/quarantine\/z/)
  })

  it('keeps the source namespace so two kinds sharing a name cannot collide', () => {
    const { script } = buildPreservationPlan([
      branch('recover-me'),
      { ...branch('recover-me'), kind: 'rescue_ref', ref: 'refs/orch-rescue/recover-me' },
    ])
    const targets = script
      .split('\n')
      .filter((l) => l.startsWith('push '))
      .map((l) => l.split(/\s+/)[2])
    assert.deepEqual(targets, ['refs/preserved/recover-me', 'refs/preserved/orch-rescue/recover-me'])
    assert.equal(new Set(targets).size, targets.length, 'preserved refs must be distinct')
  })
