...
Audit fingerprint: `1c103e3c6cc04ee0c272e6b95c99e01ee449680827f568ebdfcca678aa8c78a3`

Base: `origin/master` @ `817651866444` · generated 2026-09-10T10:03:21.567Z

Regenerate with:

```bash
node scripts/reconcile-rescue-refs.mjs --base origin/master \
  --fingerprint 1c103e3c6cc04ee0c272e6b95c99e01ee449680827f568ebdfcca678aa8c78a3 \
  --out docs/recovery-ledger-1c103e3c6cc0.json
node scripts/recovery-ledger-report.mjs --ledger docs/recovery-ledger-1c103e3c6cc0.json --project beethoven
```
...
150 item(s) below keep durable provenance in `docs/recovery-ledger-1c103e3c6cc0.json` (source ref, sha, subject, touched files, carrier branches). None were applied in this pass — per the coordination rule, conflicts get a focused follow-up rather than a forced overwrite.
...
## This ledger is byte-for-byte the same pass as `ed3b33e3e884`

Both tasks were generated from snapshots of the *same* live evidence set —
`refs/orch-rescue/*` — taken at different moments: this one recorded 617 items,
`ed3b33e3e884` recorded 589. Reconciled against the live source on 2026-09-10
they both enumerate **791 refs and produce identical counts** (241 / 365 / 35 /
150 / 0). They are one piece of work carrying two fingerprints.

That is not a one-off. At the time of writing, `beethoven` has 417
`chatgpt-local-reconcile-beethoven-*` tasks, of which **117 are still QUEUED —
86 of them over this same rescue-ref evidence**, created steadily from
2026-08-11 through today. Each costs an executor claim and roughly seven minutes
of `git apply --check` to reproduce a classification that has already been
committed, and every pass leaves its own worktree, which the next sweep then
finds as fresh evidence.

Two of the loop's feedback paths were cut in this session — `is_own_task_scratch`
(a task recovering its own report) and the `.orch-worktree.json` exclusion (23
of 30 false positives in the worktree reconciler) — but neither addresses the
generator: snapshots keep being taken of an evidence set that is already fully
classified. Deduplicating those 86 by evidence digest rather than by fingerprint
is queued as `consolidate-duplicate-rescue-ref-reconcile-tasks` and is
deliberately left as an operator-visible decision, since superseding 86 queued
tasks is not a call an executor should make on its own.
