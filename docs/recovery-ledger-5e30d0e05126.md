...
Audit fingerprint: `5e30d0e051261ecf689b4354bed3d8f89300928f4ac874e32e1be647dd45010e`

Base: `origin/master` @ `d3a6b47abff4` · generated 2026-08-16T23:29:45.066Z

Regenerate with:

```bash
node scripts/reconcile-rescue-refs.mjs --base origin/master \
  --fingerprint 5e30d0e051261ecf689b4354bed3d8f89300928f4ac874e32e1be647dd45010e \
  --out docs/recovery-ledger-5e30d0e05126.json
node scripts/recovery-ledger-report.mjs --ledger docs/recovery-ledger-5e30d0e05126.json --project beethoven
```

## Result

**550 evidence items classified, 0 UNKNOWN.** The evidence source was treated as read-only throughout — nothing was deleted, reset, cleaned, popped or moved. Classification is recomputed from live refs, not from the snapshot in the task prompt.
...
| ALREADY_PRESENT | 172 | no action — value already on the default branch |

## Items with remaining value

33 item(s) below keep durable provenance in `docs/recovery-ledger-5e30d0e05126.json` (source ref, sha, subject, touched files, carrier branches). None were applied in this pass — per the coordination rule, conflicts get a focused follow-up rather than a forced overwrite.
...
`ca93a1b7be55`, `671c267eedf3`, `10d6c3591091`, `6d094ab1edfa` and `5e30d0e05126` were
queued against this same repository and enumerate the same live evidence source, so their
classifications are identical by construction. Each carries its own ledger and its own
`coordination_tasks` rows for audit purposes; they are not five independent findings, and
the conflicted items should be de-duplicated before any follow-up work is scheduled.
