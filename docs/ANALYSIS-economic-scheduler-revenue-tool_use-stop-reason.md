...
## Re-verification stamp — 2026-08-24, rebuilt on fresh master

This document was first written on a branch that could not fast-forward onto
`orchestrator/dev`, and its DB row was later corrected to `PHANTOM_UNVERIFIED`
because it carried no `artifact_commit` — there was no sha to check out, so the
claim could not be reproduced, reverted, or audited. That is the defect this
rebuild fixes: the same 168 lines are re-committed on a branch cut from
`origin/master@c5462d62`, and every file:line citation below was re-checked
against that tree before committing rather than carried forward on trust.

Verified against `origin/master@c5462d62`:

| Claim | Check run | Result |
|---|---|---|
| `result_classifier.py:18` is the `error_max_turns` + `tool_use` conjunction | `sed -n '14,22p' runner/result_classifier.py` | confirmed verbatim at line 18 |
| `claude_cli.py` drops `subtype` and `stop_reason` when repacking `ResultMessage` | `grep -n "subtype\|stop_reason" runner/claude_cli.py` | **zero matches in the whole file** — stronger than the original claim: the fields are never read at all, so the returned dict cannot carry them |
| the repacked dict carries only text/cost/tokens/returncode/raw | `sed -n '172,205p' runner/claude_cli.py` | confirmed; `raw` holds result/cost/usage/agent_sdk/turns and nothing else |
| `result_classifier` has no production caller | `grep -rn "result_classifier" --include="*.py" .` | only `runner/result_classifier.py` itself and `tests/test_result_classifier.py` |
| `_PARKED` can shadow `_MAX_TURNS` for the same failure | `sed -n '36,52p' runner/auto_remediate.py` | confirmed: `_PARKED` (line 47) matches the literal `tool use`, and is declared **before** `_MAX_TURNS` (line 48) |

The `_PARKED`-before-`_MAX_TURNS` ordering is the sharpest of these. A run that
exhausts its turns while holding a tool-use stop reason emits prose containing
"tool use", so the parked regex claims it first and the run is filed as
near-zero-expected-value rather than as turn exhaustion — which is why the
remedy is a re-plan instead of a bigger turn budget.

Scope note: this task's prompt says explicitly *"Do not attempt a fix at this
stage; focus solely on understanding the failure."* No behaviour was changed.
The fix implied by the findings — thread `subtype`/`stop_reason` through the
`claude_cli` repack and give `result_classifier.classify()` a production caller,
rather than adding a fifteenth regex to `auto_remediate` — belongs to a
follow-up task.
