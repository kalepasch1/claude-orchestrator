...
Audit fingerprint: `bf841dc595e60029d58b4291e091878b64aa7d6d6a1c82dbb1d9021d6bd336eb`

Base: `origin/master` @ `d3a6b47abff4` · generated 2026-08-16T23:22:46.698Z

Regenerate with:

```bash
node scripts/reconcile-rescue-refs.mjs --base origin/master \
  --fingerprint bf841dc595e60029d58b4291e091878b64aa7d6d6a1c82dbb1d9021d6bd336eb \
  --out docs/recovery-ledger-bf841dc595e6.json
node scripts/recovery-ledger-report.mjs --ledger docs/recovery-ledger-bf841dc595e6.json --project beethoven
```

## Result

**547 evidence items classified, 0 UNKNOWN.** The evidence source was treated as read-only throughout — nothing was deleted, reset, cleaned, popped or moved. Classification is recomputed from live refs, not from the snapshot in the task prompt.

| Classification | Count | Disposition |
|---|---:|---|
| CONFLICTED_NEEDS_FOCUSED_TASK | 32 | queue a focused conflict-resolution task; never force-overwrite |
| ACTIVE_IN_ANOTHER_TASK | 22 | no action — leave to the owning branch/task; do not duplicate |
| SUPERSEDED_BY_NEWER | 322 | no action — newer implementation on the default branch wins |
| ALREADY_PRESENT | 171 | no action — value already on the default branch |

## Items with remaining value

32 item(s) below keep durable provenance in `docs/recovery-ledger-bf841dc595e6.json` (source ref, sha, subject, touched files, carrier branches). None were applied in this pass — per the coordination rule, conflicts get a focused follow-up rather than a forced overwrite.
...
`bf841dc595e6` and `fa219072749e` were queued against this same repository and enumerate
the same live evidence source, so their classifications are identical by construction.
Each carries its own ledger and its own `coordination_tasks` rows for audit purposes;
they are not two independent findings, and the conflicted items should be de-duplicated
before any follow-up work is scheduled.
