# chatgpt-local-reconcile-beethoven-e0945946bd0d

Reconciliation of local ChatGPT/Codex build evidence for `beethoven` against
`origin/master`, under audit fingerprint `e0945946bd0d583fcd81ae45a3663a4f97aa7f833749d2901b37ae2197946731`.

This is attempt 2. Attempt 1 failed on a merge conflict in
`docs/recovery-ledger/README.md` — two sibling reconcile tasks were each rewriting the
same shared README to describe their own run. The root cause was the deliverable shape,
not the classification: a per-task record does not belong in a file every sibling edits.
This attempt writes only files that carry the task fingerprint or slug in their name
(`.orch/recovery-ledger-e0945946bd0d.json`, `docs/tasks/chatgpt-local-reconcile-beethoven-e0945946bd0d.md`),
so concurrent siblings cannot collide. The attempt-1 tip is still reachable at
`refs/orch-preserved/chatgpt-local-reconcile-beethoven-e0945946bd0d-attempt-prior`.
...
named below is still exactly where the snapshot found it.
...
| snapshot-to-live mapping | `tools/map_snapshot_evidence.mjs` | shared with the 6c8911116873 sibling |
| ledger publication | `tools/recovery_ledger_publish.py` | yes |

The task snapshot is a digest plus a sample — 30 of 35 branch tips and 27 of 363 rescue
refs were ever recorded — so the live source was enumerated directly and is
authoritative. Each snapshot entry is then mapped onto it and gets its own ledger row.
A sampled collection rolls up over every live member of that collection, worst case
first, so the unsampled remainder is adjudicated rather than assumed clean.

## Result

Ledger: `.orch/recovery-ledger-e0945946bd0d.json` — **660 evidence items, 0 UNKNOWN**
(625 from the live enumeration, 35 snapshot entries mapped onto it).

| classification | count |
| --- | ---: |
| ALREADY_PRESENT | 233 |
| SUPERSEDED_BY_NEWER | 306 |
| ACTIVE_IN_ANOTHER_TASK | 11 |
| RECOVERABLE_VALUE | 60 |
| CONFLICTED_NEEDS_FOCUSED_TASK | 50 |
| **UNKNOWN** | **0** |
...
| `dirty_worktree` | 18 | 0 | 0 | 6 | 5 |
| `local_only_branch_tips` | 11 | 6 | 1 | 1 | 3 |
| `orchestrator_rescue_refs` | 191 | 285 | 10 | 50 | 35 |
| `snapshot:codex_output_artifact` | 0 | 0 | 0 | 1 | 0 |
| `snapshot:dirty_worktree` | 0 | 0 | 0 | 1 | 1 |
| `snapshot:local_only_branch_tips` | 12 | 15 | 0 | 0 | 3 |
| `snapshot:local_only_branch_tips_unsampled_remainder` | 0 | 0 | 0 | 0 | 1 |
| `snapshot:orchestrator_rescue_refs` | 0 | 0 | 0 | 0 | 1 |

## The five snapshot evidence items

| # | kind | source | classification |
| --- | --- | --- | --- |
| 0 | `codex_output_artifact` | `operator-output-truth-session-fabric-20260812.patch` | RECOVERABLE_VALUE |
| 1 | `dirty_worktree` | `/Users/kpasch/Documents/Codex/2026-08-07/cons/work/orchestrator-session-fabric-current` | RECOVERABLE_VALUE |
| 2 | `dirty_worktree` | `/Users/kpasch/Documents/beethoven/claude-orchestrator` | CONFLICTED_NEEDS_FOCUSED_TASK |
| 4 | `local_only_branch_tips_unsampled_remainder` | `refs/heads/` | CONFLICTED_NEEDS_FOCUSED_TASK |
| 5 | `orchestrator_rescue_refs` | `refs/orch-rescue/` | CONFLICTED_NEEDS_FOCUSED_TASK |
| 3 | `local_only_branch_tips` | 30 sampled tips, listed individually below | see breakdown |

Snapshot item 3 breakdown across its 30 sampled tips: ALREADY_PRESENT 12, SUPERSEDED_BY_NEWER 15, CONFLICTED_NEEDS_FOCUSED_TASK 3.

## Items with remaining value

Every row below is carried by this task: branch `agent/chatgpt-local-reconcile-beethoven-e0945946bd0d`,
commit: the tip of that branch, recorded verbatim in every `coordination_tasks` row.
The evidence itself is untouched — the disposition records where the value is routed,
never an instruction to delete the source.

### RECOVERABLE_VALUE (60)

Disposition for all of these: the ref, worktree or patch stays exactly where it is, and
recovery goes through a normal agent branch + merge train — one focused branch per item,
never a bulk replay over newer code. None is applied in place.
...
| `/Users/kpasch/Documents/beethoven/claude-orchestrator` | dirty_worktree | `63b610a47960` | 552 uncommitted path(s); tracked diff applies to base. Recover via a new isolated worktree + agent branch. Source worktree left untouched. |
| `/Users/kpasch/Documents/beethoven/claude-orchestrator-wt/chatgpt-local-reconcile-beethoven-84fc83c513d9` | dirty_worktree | `d3a6b47abff4` | 4 uncommitted path(s); tracked diff applies to base. Recover via a new isolated worktree + agent branch. Source worktree left untouched. |
...
| `/Users/kpasch/Documents/beethoven/claude-orchestrator-wt/chatgpt-local-reconcile-beethoven-e0945946bd0d` | dirty_worktree | `d3a6b47abff4` | 2 untracked new file(s) with no tracked counterpart; review and land through an agent branch. Source left untouched. |
...
| `snapshot[0] operator-output-truth-session-fabric-20260812.patch` | snapshot:codex_output_artifact | `` | 1 live row(s) under operator-output-truth-session-fabric-20260812.patch; worst-case classification governs the snapshot item |
| `snapshot[1] /Users/kpasch/Documents/Codex/2026-08-07/cons/work/orchestrator-session-fabric-current` | snapshot:dirty_worktree | `59de85f238e6` | 1 live row(s) under /Users/kpasch/Documents/Codex/2026-08-07/cons/work/orchestrator-session-fabric-current; worst-case classification governs the snapshot item |

### CONFLICTED_NEEDS_FOCUSED_TASK (50)

Disposition for all of these: queue a focused follow-up. None is forced onto
`origin/master`, because each either fails an apply-check against the current base or is
a snapshot entry with no base, no remote and no live counterpart. Overwriting on a guess
...
| `snapshot[2] /Users/kpasch/Documents/beethoven/claude-orchestrator` | snapshot:dirty_worktree | `6d5471008360` | 26 live row(s) under /Users/kpasch/Documents/beethoven/claude-orchestrator; worst-case classification governs the snapshot item |
| `snapshot[3] agent/dropbox-hisanta-mastery-engine-grandma-rail-family-slice-2-clean-129448` | snapshot:local_only_branch_tips | `358297faa18f` | range diff no longer applies; queue a focused follow-up rather than forcing an overwrite |
| `snapshot[3] agent/improve-missing-branch-auto-creator-slice-3-locate-decomposition-event-handler-a-clean-152441` | snapshot:local_only_branch_tips | `dc65c5428c7c` | range diff no longer applies; queue a focused follow-up rather than forcing an overwrite |
| `snapshot[3] agent/improve-value-aware-test-routing-early-exit-r-slice-3-fix-broken-tests-clean-151469` | snapshot:local_only_branch_tips | `a9e98fc3c705` | range diff no longer applies; queue a focused follow-up rather than forcing an overwrite |
| `snapshot[4] refs/heads/` | snapshot:local_only_branch_tips_unsampled_remainder | `` | 22 live row(s) under refs/heads/; worst-case classification governs the snapshot item |
| `snapshot[5] refs/orch-rescue/` | snapshot:orchestrator_rescue_refs | `` | 571 live row(s) under refs/orch-rescue/; worst-case classification governs the snapshot item |

## Coordination ledger

One `coordination_tasks` record per evidence item (660 rows), `task_type =
