...
| `runner/db_probes.py` | The probe catalog (30 probes; 28 Postgres, 9 MySQL, 3 BigQuery, 2 Snowflake). Pure SQL + deterministic parse; `score()`, `summarize()`, `validate_catalog()`. Scoring v2: per-(probe,severity) demerits decay logarithmically over repeats of one failure mode (`weight × (1 + ln n)`), then `100·e^(−D/80)` — never hard-floors at 0 (the linear v1 scored every project past ~34 mediums as 0, wiping out baseline resolution), stays monotone deep into bad posture, 1 high ≈ 88, 1 critical ≈ 73. Knob `ORCH_DB_SCORE_DECAY` (80). |
...
| `runner/db_policy_synth.py` | Candidate RLS policies from tenancy-key hints in the column inventory (facts probe `column_inventory`); drafts only, `ORCH_DB_POLICY_SYNTH=1` to emit them as PRs. |
| `runner/db_content_probe.py` | Opt-in twice (env + per-source) counts-only content checks: email-shape mismatches and secret-named columns holding data. Values never leave the database. |
| `runner/db_memo.py` (adversarial) | Every model-drafted memo argument gets an opposing-counsel attack stored as `arguments[].resilience` (`ORCH_DB_MEMO_ADVERSARIAL`). |
...
from `runner/` to apply it now rather than waiting for a cycle). Pause the whole loop
