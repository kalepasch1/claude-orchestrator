...
def _knob(name, default):
    """Read a bandit tuning knob, preferring the fleet-pushable ORCH_ name.

    CLAUDE.md requires ORCH_-prefixed keys so `fleet_control.py` can push a change
    to every machine — its `_SAFE_PREFIXES` allowlist starts with "ORCH_", and a
    bare `BANDIT_*` key is not pushable at all. But these knobs already exist under
    the bare names in deployed environments and in the recovered test suite, and
    silently ignoring an operator's existing `BANDIT_EPSILON` would be a worse
    failure than not being pushable: the runner would look configured and behave
    as if it were not.

    So both are read, ORCH_ first. New tuning goes through the fleet; existing
    local overrides keep working until they are migrated.
    """
    return os.environ.get("ORCH_" + name, os.environ.get(name, default))
...
#
# Recovered from commit 12001806 (`git show 12001806^2:runner/bandit.py`), whose
# auto-resolved merge kept the READS of these constants and dropped the block that
# defined them, leaving a module that imports with NameError. The values below are that
# branch's, verbatim — not re-derived. A bandit routing model selection on an invented
# threshold is worse than one that does not exist, because it looks like it is working.
ACCEPTANCE_MIN_SAMPLES = int(_knob("BANDIT_ACCEPT_MIN_SAMPLES", "12"))
ACCEPTANCE_CONFIDENCE = float(_knob("BANDIT_ACCEPT_CONFIDENCE", "0.95"))
# Kill switch: with this off, choose() behaves exactly as it did before the gate existed.
ACCEPTANCE_ENABLED = str(_knob("BANDIT_ACCEPTANCE", "true")).lower() in ("1", "true", "yes", "on")
