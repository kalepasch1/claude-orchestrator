...
# Acceptance-gate configuration. Read at import time (like EPSILON) so the kill switch
# is a reload away and no hot path pays for an env lookup per call.
#   BANDIT_ACCEPTANCE           kill switch — "false" restores pure UCB1 + epsilon
#   BANDIT_ACCEPT_CONFIDENCE    interval width used when comparing two arms
#   BANDIT_ACCEPT_MIN_SAMPLES   observations required behind BOTH arms before accepting
# The gate is conservative on purpose: below the sample floor, or with overlapping
# intervals, accepted_leader() returns None and exploration continues unchanged.
ACCEPTANCE_ENABLED = os.environ.get("BANDIT_ACCEPTANCE", "true").lower() in ("1", "true", "yes")
ACCEPTANCE_CONFIDENCE = float(os.environ.get("BANDIT_ACCEPT_CONFIDENCE", "0.95") or 0.95)
ACCEPTANCE_MIN_SAMPLES = int(os.environ.get("BANDIT_ACCEPT_MIN_SAMPLES", "12") or 12)
...
# Two-sided normal critical values. A small table rather than an inverse-erf so the
# module stays dependency-free and the accepted values are auditable by eye.
_Z = {
    0.80: 1.282,
    0.90: 1.645,
    0.95: 1.960,
