...
# Merge-commit subjects, verbatim as git and GitHub emit them:
#   "Merge branch 'agent/x' into master"     git merge
#   "Merge branch 'agent/x' (auto-resolved)" this fleet's auto_conflict_resolver
#   "Merge pull request #42 from o/branch"   GitHub
# Case-sensitive on purpose — see is_merge_commit().
_MERGE_MESSAGE = re.compile(r"Merge branch|Merge pull request")
...
def is_merge_commit(message: str) -> bool:
    """True when a commit message contains 'Merge branch' or 'Merge pull request'.

    Case-SENSITIVE by design. Both phrases are produced verbatim by git itself
    ('Merge branch ...') and by GitHub ('Merge pull request #N from ...'), so the exact
    casing is the signal. Lower-casing the match would classify prose about merging
    ("merge branch cleanup is overdue") as a merge commit.

    Deliberately a CONTAINS check, not a prefix or full-message match: this fleet's
    auto_conflict_resolver writes "Merge branch 'agent/x' (auto-resolved)" and the merge
    train writes multi-line messages whose first line is a merge header, so anchoring to
    the start of the string would miss real merges.

    KNOWN LIMIT, deliberately not papered over here: because it is a substring test, a
    commit that merely TALKS about a merge ("fix: handle Merge branch messages in the
    parser") matches. Message text is a heuristic, not proof. Where the repository is
    available, a commit's parent count is authoritative and should be preferred; this
    function exists for the paths that only ever see the message string.

    Args:
        message: commit message (subject alone or full body). None is treated as empty.

    Returns:
        True if either merge phrase appears in the message, False otherwise —
        including for None, non-str input, and empty/whitespace-only messages.
    """
    if not isinstance(message, str):
        return False
    return bool(_MERGE_MESSAGE.search(message))
