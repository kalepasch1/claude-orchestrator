...
class TestBillingGuardAutoResume(unittest.TestCase):
    ...
    # NOTE on patch targets in this class: billing_guard imports pause_arbiter
    # and db *inside* the functions that use them, deliberately — an
    # unimportable dependency must not break module import for the whole guard.
    # That means there is no billing_guard.pause_arbiter / billing_guard.db
    # attribute to patch; the function-local import resolves through
    # sys.modules, so the bare module target ("pause_arbiter.recheck") is the
    # one that actually intercepts the call.
