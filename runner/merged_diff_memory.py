...
def get_recent_merges(limit: int = 20, merges_only: Optional[bool] = None) -> list[dict]:
    ...
    The limit clamp is kept from the branch this was rebased onto: `merges[-limit:]` alone
    is wrong at the boundary, because -0 == 0, so a limit of 0 sliced from the start and
    returned the WHOLE list — the opposite of "give me none" — and a negative limit
    silently became "all but the first N". limit <= 0 now means no records, whichever
