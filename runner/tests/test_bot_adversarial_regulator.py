#!/usr/bin/env python3
"""Tests for adversarial_regulator bot — builds via bot_factory and asserts admission."""
...
def _mock_invoker(persona, issue):
    ...
    return {"stance": expected, "confidence": 0.86, "citations": ["mock-ref"]}

class TestAdversarialRegulatorBot(unittest.TestCase):
    def test_spec_is_valid(self):
        bf._validate_spec(SPEC)
    def test_spec_role(self):
        self.assertEqual(SPEC.role, "adversary")
    ...
    def test_admitted_with_correct_invoker(self):
        ...
        self.assertEqual(result["manifest"]["id"], "skeptical-regulator-adversary")
        self.assertEqual(result["manifest"]["role"], "adversary")
    def test_manifest_has_corpus_filter(self):
        result = bf.build_bot(SPEC, _mock_invoker)
        cf = result["manifest"]["corpus_filter"]
        self.assertIn("CFTC", cf["source"])
        self.assertIn("enforcement_action", cf["doc_types"])
    def test_priors_tag_is_skeptical_regulator(self):
        result = bf.build_bot(SPEC, _mock_invoker)
        self.assertEqual(result["manifest"]["priors_tag"], "skeptical_regulator")
