...
class AnUnslottedConsumerStillNarrowsTheCeiling(unittest.TestCase):
    """The property that makes this cap safe against things it cannot classify.

    command_builds() matches the command STRING, so it can catch a suite whose command
    IS a build or a typecheck, and can never catch one whose command is `npm test` and
    whose package script shells out to tsc. That case is real: the 5.25 GB `tsc --noEmit`
    measured on this host on 2026-09-08 was running under `tomorrow`, whose test_cmd is
    plain `npm test`. It held the second-largest block of memory on the machine and took
    no slot at all.

    Gating on free memory rather than on a count of slot-holders is what covers it. A
    heavy process that never took a slot still depresses free memory, so the ceiling
    narrows for everyone else anyway. Verified live that day: with the 5.25 GB tsc
    running and unslotted, free memory read 2.6 GB and the effective ceiling was 1.
    """

    def test_memory_consumed_outside_the_slot_system_still_lowers_the_limit(self):
        with patch.dict(os.environ, {"ORCH_MAX_CONCURRENT_BUILDS": str(TWO_BUILDS)}):
            with patch.object(build_slots, "free_gb", return_value=ROOMY_GB):
                self.assertEqual(build_slots.max_concurrent(), TWO_BUILDS)
            # Nothing took a slot; an unclassifiable process simply ate the memory.
            with patch.object(build_slots, "free_gb",
                              return_value=THE_MEASURED_PRESSURE_GB):
                self.assertEqual(build_slots.max_concurrent(), ONE_BUILD)


class TypechecksCountAsBuilds(unittest.TestCase):
    """Audited across all 16 rows of the projects table on 2026-09-08.

    Two projects run a typecheck AS their test command and neither took a slot, because
    the list only knew about bundlers. They are not cheap: apparently-archived's own
    build_cmd carries NODE_OPTIONS=--max-old-space-size=7168.
    """

    def test_the_two_project_typecheck_commands_now_take_a_slot(self):
        for command in ("npm run typecheck", "npx nuxi typecheck"):
            self.assertTrue(build_slots.command_builds(command), command)

    def test_a_direct_tsc_invocation_takes_a_slot(self):
        for command in ("tsc --noEmit --project tsconfig.json", "vue-tsc --noEmit"):
            self.assertTrue(build_slots.command_builds(command), command)

    def test_an_ordinary_suite_is_still_not_serialised(self):
        """Suites are the fleet's throughput. Widening this list must not swallow them."""
        for command in ("npm test", "npm run test", "vitest run", "pytest -q"):
            self.assertFalse(build_slots.command_builds(command), command)
