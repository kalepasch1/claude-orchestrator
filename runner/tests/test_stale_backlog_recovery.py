...
class TestBatchLimitAndFleetConfig:
    """Test batch processing limits and ORCH_* env configuration."""

    def _make_stale_task(self, n):
        return {
            'id': f'task-{n}',
            'slug': f'batch-task-{n}',
            'state': 'RUNNING',
    ...
    def test_batch_limit_constant_exists(self):
        """BATCH_LIMIT is a positive int with a sensible default."""
        if not sbr:
            pytest.skip("Module not ready")
        assert hasattr(sbr, 'BATCH_LIMIT')
        assert isinstance(sbr.BATCH_LIMIT, int)
        assert sbr.BATCH_LIMIT >= 1

    def test_pipeline_respects_batch_limit(self):
        """run_recovery_pipeline processes at most batch_limit tasks per pass."""
        if not sbr or not hasattr(sbr, 'run_recovery_pipeline'):
            pytest.skip("Module not ready")

        tasks = [self._make_stale_task(i) for i in range(10)]
        result = sbr.run_recovery_pipeline(tasks, threshold_sec=1800, batch_limit=3)
        assert result['detected_stale'] <= 3
        assert result['actions_queued'] <= 3

    def test_batch_limit_env_override(self):
        """ORCH_BACKLOG_BATCH_LIMIT env var overrides the default."""
        if not sbr:
            pytest.skip("Module not ready")
        import importlib
        with patch.dict(os.environ, {'ORCH_BACKLOG_BATCH_LIMIT': '7'}):
            importlib.reload(sbr)
            assert sbr.BATCH_LIMIT == 7
        importlib.reload(sbr)

    def test_stale_minutes_env_fallback(self):
        """ORCH_BACKLOG_STALE_MINUTES sets the threshold when the seconds var is unset."""
        if not sbr:
            pytest.skip("Module not ready")
        import importlib
        env = {'ORCH_BACKLOG_STALE_MINUTES': '5'}
        with patch.dict(os.environ, env, clear=False):
            os.environ.pop('ORCH_STALE_THRESHOLD_SECONDS', None)
            importlib.reload(sbr)
            assert sbr.STALE_THRESHOLD == 300
        importlib.reload(sbr)

    def test_seconds_env_wins_over_minutes(self):
        """ORCH_STALE_THRESHOLD_SECONDS takes precedence over the minutes fallback."""
        if not sbr:
            pytest.skip("Module not ready")
        import importlib
        env = {'ORCH_STALE_THRESHOLD_SECONDS': '900',
               'ORCH_BACKLOG_STALE_MINUTES': '5'}
        with patch.dict(os.environ, env, clear=False):
            importlib.reload(sbr)
            assert sbr.STALE_THRESHOLD == 900
        importlib.reload(sbr)

    def test_bad_env_values_fail_soft(self):
        """Malformed env values fall back to defaults instead of crashing import."""
        if not sbr:
            pytest.skip("Module not ready")
        import importlib
        env = {'ORCH_BACKLOG_BATCH_LIMIT': 'not-a-number',
               'ORCH_BACKLOG_STALE_MINUTES': 'garbage'}
        with patch.dict(os.environ, env, clear=False):
            os.environ.pop('ORCH_STALE_THRESHOLD_SECONDS', None)
            importlib.reload(sbr)
            assert sbr.BATCH_LIMIT >= 1
...
class TestOperatorHooks:
    """Test stats()/invalidate() observability hooks."""

    def test_stats_reports_applied_actions(self):
        """stats() counts actions applied via apply_recovery_action."""
        if not sbr or not hasattr(sbr, 'stats'):
            pytest.skip("Module not ready")

        sbr.invalidate()
        action = {'task_id': 't-1', 'slug': None, 'action': 'mark_stale'}
        assert sbr.apply_recovery_action(action) is True
        s = sbr.stats()
        assert s['applied_total'] == 1
        assert s['by_action'].get('mark_stale') == 1

    def test_invalidate_clears_state(self):
        """invalidate() resets the applied-actions record."""
        if not sbr or not hasattr(sbr, 'invalidate'):
            pytest.skip("Module not ready")

        sbr.apply_recovery_action({'task_id': 't-2', 'action': 'cancel'})
        sbr.invalidate()
