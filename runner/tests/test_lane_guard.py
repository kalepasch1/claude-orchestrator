...
class TestRunSupervised(unittest.TestCase):
    ...
    def test_agentic_coder_uses_the_process_group_guard(self):
        from unittest.mock import patch
        guarded = {"returncode": 0, "stdout": "done", "stderr": "",
                   "timed_out": False, "idle_killed": False,
                   "duration_s": 0.01, "reap": {}, "pid": 123}
        spec = {"name": "test-coder", "cmd": "echo {prompt}",
                "est_usd": 0, "cost": 0, "cap": 5}
        with patch.object(agentic_coders, "_spec", return_value=spec), \
             patch.object(agentic_coders, "_agentic_event"), \
             patch.object(lane_guard, "run_supervised", return_value=guarded) as run:
            result = agentic_coders.run("test-coder", "hello", "model", timeout=17)
