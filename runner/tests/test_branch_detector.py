...
class TestRemoteAwareness:
    ...
    def test_list_includes_remote_only_when_asked(self, repo):
        _make_pushed_then_pruned(repo, "shipped")
        assert "shipped" not in bd._list_agent_branches(repo)
        assert "shipped" in bd._list_agent_branches(repo, include_remote=True)
    ...
    def test_a_slug_containing_slashes_survives_the_remote_split(self, repo):
        ...
        assert "group/one" in bd._list_agent_branches(repo, include_remote=True)
