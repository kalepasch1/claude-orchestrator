...
# ── merged_diff_library.identify_affected_lines ──────────────────────────────

class IdentifyAffectedLinesTest(unittest.TestCase):
    def test_single_file_single_hunk(self):
        ...
        result = mdl.identify_affected_lines(diff)
    ...
    def test_multiple_hunks_same_file(self):
        ...
        result = mdl.identify_affected_lines(diff)
        self.assertEqual(len(result["foo.py"]), 2)
        self.assertEqual(result["foo.py"][1]["old_start"], 10)
    ...
    def test_multi_file_diff(self):
        ...
        self.assertEqual(result["b.py"][0]["old_start"], 5)

    def test_none_input_returns_empty_dict(self):
        self.assertEqual(mdl.identify_affected_lines(None), {})

    def test_empty_string_returns_empty_dict(self):
        self.assertEqual(mdl.identify_affected_lines(""), {})

    def test_non_string_input_returns_empty_dict(self):
        self.assertEqual(mdl.identify_affected_lines(12345), {})
        self.assertEqual(mdl.identify_affected_lines(["not", "a", "diff"]), {})
    ...
    def test_binary_diff_has_no_hunks(self):
        diff = (
    ...
    def test_renamed_file_with_content_change(self):
        diff = (
            "diff --git a/old.py b/new.py\n"
            "--- a/old.py\n"
            "+++ b/new.py\n"
        ...
        result = mdl.identify_affected_lines(diff)
        self.assertIn("new.py", result)
        self.assertEqual(result["new.py"][0]["old_start"], 1)

    def test_new_file_from_dev_null(self):
        diff = "--- /dev/null\n+++ b/new.py\n@@ -0,0 +1,3 @@\n+a\n+b\n+c\n"
        result = mdl.identify_affected_lines(diff)
        self.assertIn("new.py", result)
        self.assertEqual(result["new.py"][0]["old_count"], 0)

    def test_deleted_file_to_dev_null(self):
        diff = "--- a/gone.py\n+++ /dev/null\n@@ -1,3 +0,0 @@\n-a\n-b\n-c\n"
        result = mdl.identify_affected_lines(diff)
        self.assertIn("gone.py", result)
        self.assertEqual(result["gone.py"][0]["new_count"], 0)

    def test_zero_width_new_range(self):
        diff = "--- a/f.py\n+++ b/f.py\n@@ -1,3 +0,0 @@\n-a\n-b\n-c\n"
        result = mdl.identify_affected_lines(diff)
        self.assertEqual(result["f.py"][0]["new_count"], 0)

    def test_missing_newline_at_eof_does_not_break_parsing(self):
        diff = (
            "--- a/f.py\n+++ b/f.py\n@@ -1,1 +1,1 @@\n"
            "-old\n+new\n\\ No newline at end of file\n"
        )
        result = mdl.identify_affected_lines(diff)
        self.assertEqual(result["f.py"][0]["old_start"], 1)
    ...
    def test_permission_only_change_no_hunks(self):
        diff = (
        ...
        result = mdl.identify_affected_lines(diff)
        self.assertEqual(result["script.sh"], [])

    def test_no_hunks_at_all_returns_empty_dict(self):
        diff = "just some text with no diff markers at all\n"
        result = mdl.identify_affected_lines(diff)
        self.assertEqual(result, {})
    ...
    def test_malformed_hunk_header_skipped(self):
        diff = "--- a/f.py\n+++ b/f.py\n@@ garbage @@\n context\n"
        result = mdl.identify_affected_lines(diff)
        self.assertEqual(result["f.py"], [])

    def test_merge_conflict_markers_do_not_crash(self):
        diff = (
            "--- a/f.py\n+++ b/f.py\n@@ -1,3 +1,5 @@\n"
            "<<<<<<< HEAD\n+ours\n=======\n+theirs\n>>>>>>> branch\n"
        )
    ...
    def test_hunk_counts_default_to_one_when_omitted(self):
        diff = "--- a/f.py\n+++ b/f.py\n@@ -5 +5 @@\n-a\n+b\n"
        result = mdl.identify_affected_lines(diff)
        self.assertEqual(result["f.py"][0], {
            "old_start": 5, "old_count": 1, "new_start": 5, "new_count": 1
        })

    def test_no_diff_git_header_still_works(self):
        diff = "--- a/plain.py\n+++ b/plain.py\n@@ -2,1 +2,1 @@\n-old\n+new\n"
        result = mdl.identify_affected_lines(diff)
        self.assertIn("plain.py", result)
