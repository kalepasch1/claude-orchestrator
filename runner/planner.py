...
def plan(master: str, repo: str = None) -> list:
    ...
    # conflicts back into task scopes so the planner chains deps correctly.
    try:
        ...
        if predictions:
            ...
            for t in tasks:
                ...
                for hf in hot_files:
                    if hf in scope_str:
                        # Ensure tasks touching hot files are chained via deps
                        for other in tasks:
                            if other is t:
                                continue
                            ...
                            if hf in other_scope and t.get("slug") not in (other.get("deps") or []):
                                ...
                                break  # one chain link per hot file is enough
    except Exception as e:
