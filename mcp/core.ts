/**
 * HereTomorrow unified MCP server — shared core.
 *
 ...
 *
 * Transport-agnostic on purpose. server.ts serves this over stdio for Claude;
 * http.ts serves the SAME registry over Streamable HTTP for ChatGPT and any
 * other remote client. Two transports, one implementation — a second copy of
 * the tool routing would drift from this one within a week.
 *
 * Environment:
...
/**
 * Build a fully wired MCP server. Each transport gets its own instance:
 * the SDK binds one transport per Server, and an HTTP deployment serves many
 * concurrent sessions.
 */
...
/** One-line summary of what this process is serving, for startup logs. */
export function readyLine(): string {
  return (
    `${REGISTRY.size} tools across ${DIRECT_APPS.size + SEARCH_APPS.size} apps ` +
    `(${USE_DEV ? 'dev' : 'prod'})\n` +
