...
class ClassifierTests(unittest.TestCase):
    """The rule that produced the baseline — written down, so the baseline is reproducible."""

    def _runner(self, patch="", files=()):
        def fake(repo, *args, **kwargs):
            if args[:3] == ("stash", "show", "-p"):
                return patch
            if args[:2] == ("stash", "show") and "--name-only" in args:
                return "\n".join(files)
    ...
    def test_a_stash_with_no_diff_is_empty(self):
        entry = st.classify("stash@{0}", repo="/tmp", runner=self._runner(patch="   "))
        self.assertEqual(entry["bucket"], st.EMPTY)

    def test_a_stash_whose_files_touch_runner_is_flagged(self):
        entry = st.classify("stash@{0}", repo="/tmp",
                            runner=self._runner(patch="", files=["runner/db.py"]))
        self.assertTrue(entry["touches_runner"])

    def test_already_landed_is_checked_before_recoverable(self):
        """A stash already in HEAD often also applies cleanly — recovering it makes an empty commit."""
        real = st._applies
        calls = []

        def fake_applies(repo, patch, reverse=False, runner=None):
            calls.append(reverse)
            return reverse   # reverse-applies => content is in HEAD

        st._applies = fake_applies
        try:
            entry = st.classify("stash@{0}", repo="/tmp", runner=self._runner(patch="diff"))
            self.assertEqual(entry["bucket"], st.ALREADY_LANDED)
            self.assertEqual(calls[0], True, "the reverse check must come first")
        finally:
            st._applies = real

    def test_a_forward_applying_patch_is_recoverable(self):
        real = st._applies
        st._applies = lambda repo, patch, reverse=False, runner=None: not reverse
        try:
            entry = st.classify("stash@{0}", repo="/tmp", runner=self._runner(patch="diff"))
            self.assertEqual(entry["bucket"], st.RECOVERABLE)
        finally:
            st._applies = real

    def test_a_patch_that_applies_neither_way_is_conflicted(self):
        real = st._applies
        st._applies = lambda *a, **k: False
        try:
            entry = st.classify("stash@{0}", repo="/tmp", runner=self._runner(patch="diff"))
            self.assertEqual(entry["bucket"], st.CONFLICTED)
        finally:
            st._applies = real

    def test_classification_is_fail_soft(self):
        def boom(*a, **k):
            raise OSError("no git")
        entry = st.classify("stash@{0}", repo="/tmp", runner=boom)
        self.assertIn(entry["bucket"], st.BUCKETS)

    def test_triage_tallies_every_bucket_and_the_runner_subset(self):
        real = st._applies
        st._applies = lambda *a, **k: False
        try:
            report = st.triage(repo="/tmp", refs=["stash@{0}", "stash@{1}"],
                               runner=self._runner(patch="diff", files=["runner/x.py"]))
            self.assertEqual(report["total"], 2)
            self.assertEqual(report["counts"][st.CONFLICTED], 2)
            self.assertEqual(report["conflicted_touching_runner"], 2)
        finally:
            st._applies = real

    def test_triage_is_fail_soft_on_a_bad_repo(self):
        report = st.triage(repo="/nonexistent", refs=[])
        self.assertEqual(report["total"], 0)


class ReadOnlyTests(unittest.TestCase):
    def test_the_module_never_pops_applies_or_drops(self):
        source = open(os.path.join(RUNNER, "stash_triage.py"), errors="replace").read()
        for forbidden in ('"pop"', '"drop"', '"stash", "apply"', "stash pop", "stash drop"):
            self.assertNotIn(forbidden, source, f"triage must stay read-only: found {forbidden}")

    def test_the_only_git_apply_is_a_check(self):
        source = open(os.path.join(RUNNER, "stash_triage.py"), errors="replace").read()
        self.assertIn('"apply", "--check"', source)
