...
class ApprovalStatus(Enum):
    """Tri-state the gifting protocol switches on.

    Recovered from the losing side of the merge that left this file with
    conflict markers: ParentApproval.status below is annotated
    `ApprovalStatus` and defaults to `ApprovalStatus.APPROVED`, but the enum
    itself only ever existed in the other copy, so resolving in favour of this
    file produced a module that referenced an undefined name. The three members
    and their lowercase values are reproduced exactly — call sites compare
    against the members, and the values are what serialise.
    """
    APPROVED = "approved"
    DENIED = "denied"
    PENDING = "pending"
