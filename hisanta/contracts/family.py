...
class ApprovalStatus(Enum):
    """Status of a parent approval.

    Referenced by ParentApproval below but defined only in the nested copy, so this
    module — the one `hisanta.contracts.family` actually resolves to — raised
    NameError on import. Restored here, verbatim from the nested definition, so the
    two spellings cannot disagree about the values.
    """
    APPROVED = "approved"
    DENIED = "denied"
    PENDING = "pending"
...
#: The canonical export list. `hisanta/hisanta/contracts/family.py` is a re-export
#: shim that iterates this, so a symbol added here without being listed is reachable
#: by one import spelling and not the other — which is how the two copies drifted
#: apart in the first place. Add the name here when you add the definition.
__all__ = [
    "ApprovalStatus",
    "ClassroomCohort",
    "ConstitutionAction",
