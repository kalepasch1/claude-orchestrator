...
class MasteryEngine:
    ...
    def schedule_review(self, quest: Any, last_interval: int = 1, success: bool = True) -> int:
        ...
        Fail-soft on an uncoercible interval: a scheduling helper must not raise
        into the quest loop, and 1 day is the safe answer (review sooner, never
        drop the item). The diagnostic is printed rather than swallowed silently.
        """
        if not success:
            return 1
        try:
            ...
            print(f"mastery: bad last_interval {last_interval!r} ({exc}); fail-soft 1 day")
            return 1
    ...
    def create_reward_schedule(
        self, schedule_type: str = "fixed", coupled_to_purchase: bool = False
