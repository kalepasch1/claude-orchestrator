...
consumer imports `hisanta.contracts.family` (absolute) — which `hisanta/__init__.py`
resolves to the top-level directory, since the nested tree is *appended* to
`__path__` and can never shadow it — the nested definitions were unreachable dead
code that still had to be kept in sync by hand.
