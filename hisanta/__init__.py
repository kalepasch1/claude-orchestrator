...
ModuleNotFoundError from the other. Extending ``__path__`` over the nested tree
makes both spellings resolve without moving any file or breaking the existing
...
bare ``__path__.append(...)`` reads a name a static checker cannot see defined
here, which is what tripped the pyflakes undefined-name guard.
"""
import os as _os
