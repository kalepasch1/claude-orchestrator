...
`hisanta.mastery`, ... Which directory `hisanta` resolves to also depends on which
rootdir ends up on sys.path, so `import hisanta.mastery.engine` worked from one and
...
any file: `hisanta.mastery` and `hisanta.hisanta.mastery` are the same subpackage, and
the existing `hisanta.hisanta.mastery.engine` imports in school/classroom.py keep
working.
