...
create table if not exists growth_budget (
  scope text not null, key text not null, allocation numeric(14,2) not null default 0,
  spend numeric(14,2) not null default 0, cap numeric(14,2), status text not null default 'active',
...
begin
  for rec in select scope,key,spend from growth_budget where status='active' and spend >= p_min_spend loop
    if rec.scope='app' then
      select count(*) into convs from growth_events where app=rec.key and event_type in ('signup','revenue') and ts >= now()-p_window;
    else
      select count(*) into convs from growth_events where segment=rec.key and event_type in ('signup','revenue') and ts >= now()-p_window;
    end if;
    if convs = 0 then
...
select actor_hash, score_lead(actor_hash) as score, max(ts) as last_seen, count(*) as touches
...
returns text language sql stable as $$
  select r.to_app from growth_events e
  ...
  group by r.to_app order by count(*) desc limit 1;
$$;
...
begin
  ...
  if pid is not null then
    ...
    values (pid, 'case-study-'||left(p_actor_hash,10)||'-'||to_char(now(),'YYYYMMDD'),
      format('Draft a case study + testimonial request for a %s win in segment %s (value %s). Use only approved, attestable facts; request the customer consent + quote; on approval publish and feed the content engine.', p_app, p_segment, p_value),
  ...
  values (p_app, 'claim', p_actor_hash, 'Customer success outcome (pending consent + verification)',
    jsonb_build_object('segment',p_segment,'value',p_value), 'draft') returning id into att;
