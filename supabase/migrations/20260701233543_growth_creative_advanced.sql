...
returns jsonb language sql stable as $$
  select jsonb_build_object(
    ...
    'brand_masters', (select coalesce(jsonb_agg(jsonb_build_object('title',title,'gen_prompt',gen_prompt,'asset_url',asset_url,'kind',kind)),'[]')
...
create table if not exists growth_safety_check (
  ...
  check_type text not null, result text not null, detail text, created_at timestamptz not null default now()
);
...
returns boolean language sql stable as $$
  select not exists (
    ...
    where c.arm_id = p_arm_id and (c.status not in ('approved','published') or not safety_ok(c.id))
  );
$$;
...
begin
  ...
  values (seg, 'creative-'||left(p_creative_id::text,8), jsonb_build_object('creative_id',p_creative_id,'asset_url',cr.asset_url,'kind',cr.kind))
  on conflict (segment_id, arm) do update set variant=excluded.variant returning id into aid;
...
select app, count(*) filter (where role='master') as masters,
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object('id',id,'kind',kind,'derivative_kind',derivative_kind,'asset_url',asset_url,'status',status)),'[]')
    into assets from growth_creative where (master_id = p_master_id or id = p_master_id) and status in ('approved','published');
...
begin
  ...
  mid := register_master(p_target_app, p_segment, 'Adapted: '||pl.name,
