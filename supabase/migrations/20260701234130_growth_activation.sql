...
begin
  for rec in select * from growth_ad_spend where not synced loop
    ...
    update growth_budget set spend = spend + coalesce(rec.spend,0), updated_at=now() where scope='app' and key=rec.app;
    if rec.arm_id is not null then
      update growth_arms set impressions = impressions + rec.impressions,
        conversions = conversions + rec.conversions, reward_sum = reward_sum + coalesce(rec.revenue,0) where id = rec.arm_id;
    end if;
    update growth_ad_spend set synced = true where id = rec.id;
    n := n + 1;
  end loop;
  perform check_cac_circuit();
  return n;
end $$;
...
select c.id, c.app, c.segment, c.title, c.cost_usd, a.impressions, a.conversions, a.reward_sum as revenue,
       round(a.reward_sum - c.cost_usd, 2) as net,
       case when c.cost_usd>0 then round(a.reward_sum / c.cost_usd, 2) else null end as roas
from growth_creative c join growth_arms a on a.id = c.arm_id where c.status in ('approved','published');
...
begin
  ...
  select 'channel:'||coalesce(s.channel,'na'),
         round(avg(case when a.impressions>0 then a.conversions::numeric/a.impressions end),4), sum(a.impressions), now()
  from growth_arms a join growth_segments s on s.id=a.segment_id group by coalesce(s.channel,'na')
  on conflict (feature_key) do update set expected_lift=excluded.expected_lift, samples=excluded.samples, updated_at=now();
  get diagnostics n = row_count; return n;
end $$;
create or replace function prior_for(p_feature_key text)
returns numeric language sql stable as $$ select expected_lift from growth_world_model where feature_key = p_feature_key; $$;
create or replace function sim_gate(p_segment text, p_min_conv numeric default 0.02)
returns boolean language sql stable as $$ select exists (select 1 from growth_sim where segment = p_segment and predicted_conv >= p_min_conv); $$;
...
select 'reply_triage' as surface, count(*) filter (where auto_handled) as auto_actions,
       count(*) filter (where auto_handled and human_edited) as overridden,
       round(1 - count(*) filter (where auto_handled and human_edited)::numeric / greatest(count(*) filter (where auto_handled),1),3) as accuracy
from growth_reply_triage
union all
select 'creative_auto_triage', count(*) filter (where source='ai'), count(*) filter (where source='ai' and status='rejected'),
       round(1 - count(*) filter (where source='ai' and status='rejected')::numeric / greatest(count(*) filter (where source='ai'),1),3)
...
declare tbl text;
begin
  foreach tbl in array array['growth_ad_accounts','growth_ad_spend','growth_world_model','growth_market_signal','growth_provenance','growth_tenant'] loop
