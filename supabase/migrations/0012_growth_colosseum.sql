...
-- The Marketing Colosseum: an evolutionary market of competing strategy agents.
-- Proposals reduce to arms (0010) and win on live evidence; strategists earn ELO + calibration.
-- Idempotent.

-- Strategists = versioned personas (great marketing minds + AI-native lenses).
create table if not exists growth_strategists (
  id           uuid primary key default gen_random_uuid(),
  handle       text unique not null,
  display_name text not null,
  lens         text,                                   -- the school / technique
  objective    text not null default 'acquisition',    -- acquisition|activation|retention|monetization
  policy       jsonb not null default '{}',            -- prompt/persona config
  elo          numeric(8,2) not null default 1200,
  calibration  numeric(5,4) not null default 0.5000,   -- EMA of (1 - Brier); higher = better forecaster
  budget_credits numeric(12,2) not null default 100,
  autonomy     numeric(4,3) not null default 0,        -- governance dial 0..1 (auto-raised by track record)
  wins         int not null default 0,
  losses       int not null default 0,
  pnl          numeric(14,2) not null default 0,
  status       text not null default 'active',         -- active|benched|retired
...
create table if not exists growth_tournaments (
  id           uuid primary key default gen_random_uuid(),
  objective    text not null default 'acquisition',
  target_app   text,
  target_segment text,
  round        int not null default 1,
  status       text not null default 'open',           -- open|allocating|running|settled
...
create table if not exists growth_proposals (
  id            uuid primary key default gen_random_uuid(),
  tournament_id uuid references growth_tournaments(id) on delete cascade,
  strategist_id uuid references growth_strategists(id) on delete set null,
  hypothesis    text,
  predicted_lift numeric(8,4) default 0,               -- strategist's forecast (relative lift vs baseline)
  rationale     text,
  arm_id        uuid references growth_arms(id) on delete set null,
  critique_score numeric(5,2),
  allocation    numeric(6,4) not null default 0,        -- share of traffic/budget
  status        text not null default 'submitted',      -- submitted|cut|live|settled
  realized_lift numeric(8,4),
  created_at    timestamptz not null default now()
);
...
create table if not exists growth_strategist_scores (
  ...
  strategist_id uuid references growth_strategists(id) on delete cascade,
  proposal_id   uuid references growth_proposals(id) on delete cascade,
  predicted_lift numeric(8,4),
  realized_lift  numeric(8,4),
  brier          numeric(6,4),
  pnl            numeric(14,2),
  created_at     timestamptz not null default now()
...
-- open a tournament for a segment (baseline = current pooled conversion rate of the segment)
...
begin
  ...
  select case when sum(impressions)>0 then sum(conversions)::numeric/sum(impressions) else 0 end
    into base from growth_arms where segment_id = seg;
  insert into growth_tournaments(objective, target_app, target_segment, baseline_rate, meta)
  values (p_objective, p_app, p_segment_path, coalesce(base,0), jsonb_build_object('segment_id', seg))
  returning id into tid;
  return tid;
end $$;

-- allocate traffic/budget across live proposals: softmax over strategist ELO * (1 + predicted_lift),
-- tempered by critique_score. Cuts the long tail below a floor.
...
begin
  with ranked as (
    select pr.id,
      s.elo * (1 + greatest(pr.predicted_lift,0)) * (0.5 + coalesce(pr.critique_score,50)/100.0) as w,
  ...
  update growth_proposals pr
    set allocation = case when pr.id in (select id from kept)
                          then round((select w from kept where kept.id=pr.id)/nullif((select sw from tot),0),4)
                          else 0 end,
...
-- settle: score each live proposal on realized lift vs baseline, update ELO + calibration + pnl,
-- crown the winner, and bank the winning arm as a reusable play. Returns winning proposal id.
...
declare t growth_tournaments; field_avg numeric; rec record; winner uuid; best numeric := -1e9;
  v_rate numeric; v_lift numeric; v_actual numeric; v_expected numeric; v_brier numeric;
  v_np numeric; v_nr numeric; play uuid;
begin
  ...
  for rec in
    ...
    from growth_proposals pr
    join growth_arms a on a.id = pr.arm_id
    join growth_strategists s on s.id = pr.strategist_id
    where pr.tournament_id = p_tournament_id and pr.status='live'
  loop
    ...
    -- Brier on normalized lift ([-1,3] -> [0,1])
    ...
    update growth_strategists set
      ...
      budget_credits = greatest(0, budget_credits + 10*(v_actual - v_expected)),
      updated_at = now()
    where id = rec.strategist_id;
    -- governance dial from refreshed stats
    update growth_strategists set
      autonomy = least(1, greatest(0, ((elo-1200)/600.0)*0.5 + calibration*0.5))
    where id = rec.strategist_id;
  ...
  -- bank the winning arm as a reusable play if it beat baseline
  if winner is not null and best > 0 then
    select promote_arm_to_play(pr.arm_id,
       'colosseum:'||t.target_app||':'||to_char(now(),'YYYY-MM-DD')||':'||left(pr.id::text,8), 'creative')
      into play from growth_proposals pr where pr.id = winner;
  end if;

  update growth_tournaments set status='settled', settled_at=now(),
    meta = meta || jsonb_build_object('winner_proposal', winner, 'winning_lift', best, 'play', play)
...
-- leaderboard view for Smarter
create or replace view growth_leaderboard as
select handle, display_name, lens, objective, elo, calibration, wins, losses,
       round(pnl,0) pnl, round(autonomy,3) autonomy, budget_credits, status
...
-- schedule a Colosseum round cadence (runner reads loops; type is free text)
