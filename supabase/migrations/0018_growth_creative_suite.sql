...
-- Creative/design review suite: every non-text asset (image/graphic/branding) is brand-checked and
-- must be APPROVED by a designer before it can go live. Brand kit = single source of truth so the
-- designer improves the whole system, not one asset at a time. Idempotent.

-- Brand kit (the design system per app): palette, fonts, logo rules, tone, distinctive assets, do/don't.
...
-- Creative asset (AI- or human-made). status is the review lifecycle.
create table if not exists growth_creative (
  id         uuid primary key default gen_random_uuid(),
  app        text not null,
  segment    text,
  arm_id     uuid references growth_arms(id) on delete set null,  -- if it backs a bandit arm
  kind       text not null default 'image',   -- image|logo|banner|social|ad|deck|brand_asset
  title      text,
  gen_prompt text,                             -- the prompt used to generate it (for regen)
  asset_url  text,
  thumb_url  text,
  source     text not null default 'ai',       -- ai|human|stock
  brand_score numeric(5,3),                    -- auto vision-model score vs brand kit (0..1)
  status     text not null default 'draft',    -- draft|in_review|changes_requested|approved|rejected|published
...
-- Review trail (approvals, change requests, per-region annotations for auto-regeneration).
create table if not exists growth_creative_review (
  id          bigint generated always as identity primary key,
  creative_id uuid references growth_creative(id) on delete cascade,
  reviewer    text,
  decision    text not null,                   -- approve|request_changes|reject
  comments    text,
  annotations jsonb not null default '[]',     -- [{region, note}] -> feeds regeneration
  created_at  timestamptz not null default now()
);

-- Designer decision -> update lifecycle. request_changes carries annotations for auto-regen.
...
begin
  ...
  update growth_creative set
    status = case p_decision when 'approve' then 'approved'
...
-- Auto-triage on the vision-model brand score: obvious on-brand auto-approves, obvious off-brand
-- auto-rejects, borderline goes to the designer. The designer sets the thresholds (governs the firehose).
...
-- GATE: a visual arm may only serve once its creative is approved/published (text-only arms unaffected).
create or replace function creative_gate(p_arm_id uuid)
returns boolean language sql stable as $$
  select not exists (
    select 1 from growth_creative
    where arm_id = p_arm_id and status not in ('approved','published')
  );
$$;
...
declare v int;
begin
  insert into growth_brand_kit(app, spec, version, updated_by, updated_at)
  values (p_app, p_spec, 1, p_by, now())
  on conflict (app) do update set spec=excluded.spec, version=growth_brand_kit.version+1,
    updated_by=excluded.updated_by, updated_at=now()
  returning version into v;
  return v;
end $$;

-- the designer's queue in Smarter
create or replace view growth_design_queue as
select c.id, c.app, c.segment, c.kind, c.title, c.asset_url, c.thumb_url, c.brand_score, c.status,
       c.gen_prompt, c.created_at,
       (select decision from growth_creative_review r where r.creative_id=c.id order by created_at desc limit 1) as last_decision
from growth_creative c
where c.status in ('in_review','changes_requested')
...
-- seed starter brand kits (designer edits these in Smarter)
