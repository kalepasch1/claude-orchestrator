...
-- Smarter gating + closing the human loops + compounding intelligence. Idempotent.
--  A policy-as-approval   B auto-rollback/anomaly   C per-strategist send authority
--  D meeting loop  E reply-triage tiers  F win/loss autopsy  G profit-weighted budget  H operator week

-- A) POLICY-AS-APPROVAL: a human writes NL policy; an engine compiles it to rules stored here.
create table if not exists growth_autonomy_policy (
  id uuid primary key default gen_random_uuid(),
  policy_text text not null,
  compiled jsonb not null default '[]',   -- [{effect:'allow|flag|deny', segment_like, max_value, block_keywords[]}]
...
begin
  ...
  if rules is null then return 'allow'; end if;            -- no policy => defer to switches only
  for r in select * from jsonb_array_elements(rules) loop
    ...
    -- keyword rules are pure guards: hard-deny on match, otherwise fall through to later rules
    if jsonb_array_length(coalesce(r->'block_keywords','[]')) > 0 then
      for kw in select jsonb_array_elements_text(r->'block_keywords') loop
        if p_text ilike '%'||kw||'%' then return 'deny'; end if;
      end loop;
      continue;
    end if;
    -- value-bounded rules only apply within their bound
    if (r->>'max_value') is not null and p_value > (r->>'max_value')::numeric then continue; end if;
    return coalesce(r->>'effect','allow');
  end loop;
  return 'allow';
end $$;

-- B) REVERSIBLE AUTONOMY + AUTO-ROLLBACK
create table if not exists growth_action_log (
  id uuid primary key default gen_random_uuid(), app text, kind text, ref text,
  undo_until timestamptz, undone boolean not null default false, meta jsonb not null default '{}',
...
returns numeric language sql stable as $$
  ...
  select round(1.0 - (
     ...
     / greatest((select count(*) from e where event_type in ('outreach_sent','visit')),1)
  ), 3);
$$;
-- pause campaigns whose health drops below threshold (auto-rollback), returns count paused
...
begin
  for rec in select key from growth_autonomy_switch where scope='campaign' and mode <> 'off' loop
    ...
    if sent >= p_min_sent and campaign_health(rec.key) < p_health_floor then
      ...
      insert into growth_action_log(app, kind, ref, meta)
        values (null,'auto_rollback', rec.key, jsonb_build_object('health', campaign_health(rec.key)));
      insert into growth_human_queue(app, kind, title, why, segment, prepared)
        values ('portfolio','approval','Auto-paused campaign: '||rec.key,
                'Health fell below floor; review before resuming.', rec.key,
                jsonb_build_object('health', campaign_health(rec.key)));
      n := n + 1;
    end if;
  end loop; return n;
end $$;

-- C) PER-STRATEGIST SEND AUTHORITY (earned by ELO + calibration)
...
returns boolean language sql stable as $$
  select coalesce((select autonomy >= p_min_autonomy and calibration >= p_min_calibration
                   from growth_strategists where id = p_strategist_id), false);
$$;

-- D) MEETING LOOP (book -> brief -> record -> auto-follow-up)
create table if not exists growth_meetings (
  id uuid primary key default gen_random_uuid(), app text, segment text, actor_hash text,
  scheduled_at timestamptz, status text not null default 'proposed', -- proposed|booked|held|no_show
  brief jsonb not null default '{}', transcript text, followup_draft text,
  created_at timestamptz not null default now()
);
...
begin
  ...
  insert into growth_outreach(app, actor_hash, segment, state, next_action_at, last_channel,
    meta) values (m.app, m.actor_hash, m.segment, 'queued', now(), 'email',
    jsonb_build_object('reason','post_meeting_followup','meeting', p_meeting_id))
  returning id into oid;
  return oid;
end $$;

-- E) REPLY-TRIAGE CONFIDENCE TIERS (auto-handle high confidence; flag judgment calls; self-tighten)
...
-- observed auto-handle accuracy => the threshold self-tightens as edit rate rises
create or replace view growth_triage_calibration as
select app,
  count(*) filter (where auto_handled) as auto_handled,
  count(*) filter (where auto_handled and human_edited) as auto_overridden,
  round(1.0 - count(*) filter (where auto_handled and human_edited)::numeric
        / greatest(count(*) filter (where auto_handled),1), 3) as auto_precision,
  -- recommended min confidence: raise it when precision is low
  round(greatest(0.6, 1.0 - (count(*) filter (where auto_handled and human_edited)::numeric
        / greatest(count(*) filter (where auto_handled),1))), 3) as recommended_min_confidence
from growth_reply_triage group by app;

-- F) WIN/LOSS AUTOPSY (feeds strategists + product roadmap)
create table if not exists growth_autopsy (
  id uuid primary key default gen_random_uuid(), app text, segment text, outcome text, -- win|loss
...
from growth_autopsy where outcome='loss' group by app, segment, jsonb_array_elements_text(reasons)
order by count(*) desc;

-- G) PROFIT-WEIGHTED BUDGET (RL on realized profit, not just momentum)
...
begin
  create temp table _p on commit drop as
    select app, coalesce(sum(value),0) as rev from growth_events
     where event_type in ('revenue','expansion') and ts >= now()-p_window group by app;
  select sum(rev) into tot from _p;
  if coalesce(tot,0) = 0 then return rebalance_budget(p_total); end if;  -- fall back to momentum
  for rec in select app, rev from _p where rev > 0 loop
    insert into growth_budget(scope,key,allocation,updated_at)
    values ('app', rec.app, round(p_total*rec.rev/tot,2), now())
...
-- H) OPERATOR ATTENTION ROUTER (books the human's highest-value week)
create table if not exists growth_operator_agenda (
  id uuid primary key default gen_random_uuid(), for_week text, priority int, kind text,
  title text, why text, ref text, est_value numeric default 0, created_at timestamptz not null default now()
);
...
begin
  ...
  -- meetings first, then presentations, then flagged material/approvals
  insert into growth_operator_agenda(for_week, priority, kind, title, why, ref)
  select wk, case h.kind when 'meeting' then 1 when 'presentation' then 2 else 3 end,
         h.kind, h.title, h.why, h.id::text
  from growth_human_queue h where h.status='open';
  get diagnostics n = row_count;
  -- one deep-work focus block on the top-momentum app
  select display_name into top from growth_momentum_latest where tier<>'infra' order by rank limit 1;
  if top is not null then
    insert into growth_operator_agenda(for_week, priority, kind, title, why)
    values (wk, 0, 'focus', 'Deep-work: '||top, 'Highest portfolio momentum this week');
    n := n + 1;
  end if;
  return n;
end $$;
...
declare tbl text;
begin
  foreach tbl in array array['growth_autonomy_policy','growth_action_log','growth_meetings',
                             'growth_reply_triage','growth_autopsy','growth_operator_agenda'] loop
