-- per-app autonomous loops the orchestrator manages (loop on a loop)
create table if not exists loops (
  id uuid primary key default gen_random_uuid(),
  project text not null,
  type text not null,                 -- learn | remediate | optimize | review
...
-- resource governor events (keep the Mac alive)
create table if not exists resource_events (
  id bigint generated always as identity primary key,
  kind text not null,                 -- disk | ram | prune | throttle
  value numeric, detail text, action text,
  created_at timestamptz not null default now()
);
-- next-step decisions from watching sessions pause/finish
create table if not exists session_actions (
  id uuid primary key default gen_random_uuid(),
  session_id text, project text, status text,   -- paused | finished
...
alter publication supabase_realtime add table session_actions;;
