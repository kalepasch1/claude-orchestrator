...
-- Mailbox management + provisioning, product/service→app offerings, and NL campaign inception.
-- Idempotent.

-- Mailbox mgmt: label + purpose + owner (personal vs bot-operated).
alter table growth_mailbox add column if not exists label text;
alter table growth_mailbox add column if not exists purpose text not null default 'outreach';  -- outreach|personal|bots
alter table growth_mailbox add column if not exists owner text not null default 'user';          -- user|bots
alter table growth_mailbox add column if not exists enabled boolean not null default true;

-- Provisioning requests for NEW mailboxes. NOTE: consumer Gmail/Outlook cannot be auto-created via
-- API (ToS). This provisions addresses under a Google Workspace / Microsoft 365 domain you own
-- (Admin SDK / Graph) OR creates send-as aliases on an existing inbox. Connector fills created_address.
create table if not exists growth_mailbox_provision (
  id uuid primary key default gen_random_uuid(),
  provider text not null default 'gmail',        -- gmail(workspace) | outlook(365)
  method text not null default 'workspace_user',  -- workspace_user | alias | send_as
  domain text, desired_local text, display_name text, purpose text default 'outreach', owner text default 'bots',
  status text not null default 'requested',       -- requested|provisioning|created|failed
  created_address text, detail text, created_at timestamptz not null default now()
);

-- Offerings: each product/service assigned to an app/project.
create table if not exists growth_offering (
  ...
  kind text not null default 'service',           -- product|service
  description text, status text not null default 'active', created_at timestamptz not null default now(),
  unique (app, name)
);

-- NL campaign inception: turn a parsed spec into a staged (NOT launched) campaign + segment + master.
-- Autonomy stays OFF; nothing sends. brand_mode: firm|personal|none.
...
begin
  ...
  values (app_, seg, title, coalesce(p_spec->>'objective','acquisition'), mid, 'draft')
