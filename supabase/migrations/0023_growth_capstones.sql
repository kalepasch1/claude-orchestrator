...
-- Capstones that make the existing system smarter, not bigger: first-class campaigns, a growth eval
-- harness, counterfactual value accounting, the compounding-dividend job, and a budget self-governor.
-- Idempotent.

-- A) FIRST-CLASS CAMPAIGNS: tie segment + creative master + autonomy into one launchable object.
create table if not exists growth_campaign (
  ...
  objective text default 'acquisition', master_id uuid, status text not null default 'draft', -- draft|staged|active|paused|done
  created_at timestamptz not null default now()
);
-- Launch = set the campaign-scope switch for its segment. Global switch still governs (ships OFF),
-- so this cannot send on its own; mode 'approval' means every contact still needs a human OK.
...
-- B) GROWTH EVAL HARNESS: golden cases + runs, so autonomous decisions are continuously benchmarked.
create table if not exists growth_eval_case (
  id uuid primary key default gen_random_uuid(), suite text not null, input jsonb not null,
  expected jsonb not null, created_at timestamptz not null default now()
);
create table if not exists growth_eval_run (
  id bigint generated always as identity primary key, suite text, score numeric(5,3), passed int, total int,
  detail jsonb not null default '{}', created_at timestamptz not null default now()
);

-- C) COUNTERFACTUAL VALUE: incremental conversions the system created vs a do-nothing baseline
-- (per segment: winning-arm rate minus the segment's control/pooled rate, x impressions).
...
-- D) COMPOUNDING DIVIDEND: propose the single best proven play into every app that lacks it.
...
-- E) BUDGET SELF-GOVERNOR: the system proposes its own budget from realized revenue + momentum.
...
begin
  select coalesce(sum(value),0) into rev from growth_events
    where event_type in ('revenue','expansion') and ts >= now()-interval '30 days';
  select coalesce(avg(score),0) into mom from growth_momentum_latest where tier<>'infra';
  proposed := round(rev * p_factor, 2);
  return jsonb_build_object('proposed_total', proposed, 'basis_revenue_30d', rev,
    'avg_momentum', round(mom,1), 'note', 'human approves before spend; circuit breaker + switches still apply');
end $$;
