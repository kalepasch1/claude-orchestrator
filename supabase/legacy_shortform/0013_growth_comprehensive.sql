...
-- The 10 additive/novel improvements from COLOSSEUM.md, built comprehensively. Idempotent.
--  1 competitor sensing · 2 synthetic-audience pretest · 3 prediction market · 4 VoC mining
--  5 retention colosseum · 6 asset-level creative · 7 pricing experiments · 8 attribution
--  9 self-discovering ICPs · 10 governance trust dial

-- 1) COMPETITOR SENSING (runner fills via Claude-in-Chrome; strategists read)
...
-- 2) SYNTHETIC-AUDIENCE PRE-TESTING (kill losers before real traffic)
...
select proposal_id, round(avg(score),4) avg_score, count(*) n_personas,
       bool_or(verdict='pass') any_pass
from growth_synth_tests group by proposal_id;
-- gate: does a proposal clear the synthetic pre-screen?
...
-- 3) PREDICTION MARKET among strategists (wisdom-of-agents prior for allocation)
...
-- 4) VOICE-OF-CUSTOMER MINING (copy in customers' own words)
...
-- 5) RETENTION / EXPANSION COLOSSEUM (objective already parameterized on tournaments/strategists)
...
-- 6) ASSET-LEVEL CREATIVE (full generated landing/email/ad variants, each tied to an arm)
...
-- 7) PRICING EXPERIMENTS (price is just an arm variant; results view = revenue per visitor by price)
...
-- 8) CLOSED-LOOP ATTRIBUTION (linear credit across channels for converting actors, cross-app)
...
-- 9) SELF-DISCOVERING ICPs (find high-converting cohorts you never authored, propose segments)
...
begin
  for rec in
    ...
    where e.app = p_app and e.channel is not null
...
-- 10) GOVERNANCE TRUST DIAL (autonomy lowers the human-approval bar for proven strategists)
...
begin
  ...
  if au >= coalesce((cfg->>'min_autonomy')::numeric,0.6)
     and p_spend <= coalesce((cfg->>'max_auto_spend')::numeric,500) then
    return false;   -- auto-approved: proven strategist, small spend
  end if;
  return true;      -- needs human sign-off in Smarter
end $$;
...
declare tbl text;
begin
  foreach tbl in array array['growth_competitor_intel','growth_synth_tests','growth_wagers','growth_voc',
                             'growth_assets','growth_governance'] loop
