...
-- Serial-builder superpowers: export/import a portfolio "growth genome" to seed a new app's GTM on
-- day one; the growth->product backlog loop; creative-play propagation; sim + board-memo stores.
-- Idempotent.

-- ---- Growth genome: a portable bundle of everything that works ----
...
returns jsonb language sql stable as $$
  select jsonb_build_object(
    ...
    'segments', (select coalesce(jsonb_agg(jsonb_build_object('path',path,'positioning',positioning,
                    'message',message,'channel',channel,'offer',offer)),'[]')
                 from growth_segments where app=p_app),
    ...
    'playbooks', (select coalesce(jsonb_agg(jsonb_build_object('segment',segment,'channel',channel,
                  ...
                  from growth_bd_playbook where status='active' order by score desc),
    'policy', (select compiled from growth_autonomy_policy where active order by created_at desc limit 1)
  );
$$;

-- Seed a NEW app from a genome: brand kit + segments (path remapped) + playbooks. Autonomy stays OFF.
...
-- Aggregate demand signals (VoC + loss reasons + unmet corpus demand) into a ranked backlog.
create or replace function build_product_backlog(p_app text)
returns int language plpgsql as $$
declare n int := 0; rec record;
begin
  -- top voice-of-customer phrases
  for rec in select phrase, sum(freq) f from growth_voc where app=p_app group by phrase order by sum(freq) desc limit 10 loop
    insert into growth_product_backlog(app, source, item, impact)
    values (p_app,'voc', rec.phrase, rec.f) on conflict do nothing; n := n+1;
  end loop;
  ...
  for rec in select reason, count(*) c from growth_autopsy, jsonb_array_elements_text(reasons) reason
    ...
    insert into growth_product_backlog(app, source, item, impact)
    values (p_app,'loss_reason', rec.reason, rec.c*3) on conflict do nothing; n := n+1;
  end loop;
  -- unmet content/corpus demand
  for rec in select topic, gap_demand from growth_content where app=p_app and status <> 'published'
             order by gap_demand desc limit 10 loop
    insert into growth_product_backlog(app, source, item, impact)
    values (p_app,'demand_gap', rec.topic, rec.gap_demand) on conflict do nothing; n := n+1;
  end loop;
  return n;
end $$;

-- ---- Creative-play propagation: an approved, proven visual becomes reusable across apps ----
...
begin
  ...
  values (p_name, 'creative', cr.app, cr.segment,
    jsonb_build_object('asset_url',cr.asset_url,'gen_prompt',cr.gen_prompt,'kind',cr.kind,'brand_score',cr.brand_score),
    'proven') returning id into pid;
  return pid;
end $$;

-- ---- Stores for the digital-twin simulator + portfolio board memo (engine-populated) ----
