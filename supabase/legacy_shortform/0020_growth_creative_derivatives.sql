...
-- Designer drafts a MASTER artifact for a new campaign/brand; bots ingest it and morph it into every
-- form + invent novel extensions, all on-brand. Plus brand-health/drift, creative performance,
-- designer-as-teacher (RLHF) feedback, format matrix, and role routing. Idempotent.

-- Extend creatives with master/derivative lineage + cost.
alter table growth_creative add column if not exists master_id uuid references growth_creative(id) on delete set null;
alter table growth_creative add column if not exists role text not null default 'standalone'; -- master|derivative|standalone
...
-- The matrix of forms a master expands into (channels/sizes/locales). Seed the common set.
...
-- Designer registers a master; bots then expand it. (Insert done by the app; helper for completeness.)
...
-- Morph a master into every active form + N novel extensions. Creates draft derivatives (inheriting
-- brand/app/segment) and files a gtm generation task for each. Returns count queued.
...
begin
  ...
  -- 2) novel extensions (bots invent new on-brand forms the matrix doesn't list)
  ...
  if pid is not null then
    ...
    values (pid, 'creative-derivatives-'||left(p_master_id::text,8),
      ...
      'gtm','QUEUED','auto-filed by request_derivatives')
    on conflict do nothing;
  end if;
  return n;
end $$;

-- Designer-as-teacher: capture every approve/reject/edit as an RLHF signal to tune generation.
...
-- Brand-drift monitor: brand health per app.
...
-- Creative performance (design ROI): join creatives to the arms they back.
...
-- Role routing (designer vs copywriter vs compliance queues).
