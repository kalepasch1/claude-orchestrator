-- v3.7: secrets (REFERENCES only — never values), kill switch, external provider usage/budgets,
-- credential onboarding. Spend governance + security controls for all external subscriptions/APIs.
...
-- secrets: RLS ON, NO policies => service-role only (the runner). Web/anon cannot read refs.
