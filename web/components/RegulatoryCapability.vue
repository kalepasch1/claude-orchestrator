...
type View = 'coverage' | 'foresight' | 'qualify' | 'evidence' | 'agreements' | 'relationships' | 'options' | 'settings'
  ...
  <section class="regulatory" aria-labelledby="regulatory-title">
    ...
    <div v-if="state" class="shell">
      <nav aria-label="Regulatory capability">
        <button v-for="item in [{id:'coverage',label:'Coverage'},{id:'foresight',label:'Foresight'},{id:'qualify',label:'Qualify'},{id:'evidence',label:'Evidence'},{id:'agreements',label:'Agreements'},{id:'relationships',label:'Relationships'},{id:'options',label:'Options'},{id:'settings',label:'Controls'}]" :key="item.id" :class="{active:view===item.id}" @click="view=item.id as View">{{ item.label }}</button>
      </nav>
