...
<template>
  <section class="cade-os" aria-labelledby="cade-os-title">
    <header class="cade-os__header">
      <div>
        <div class="cade-os__eyebrow">Madeus decision intelligence</div>
    ...
    <div v-if="active === 'replay'" class="panel replay">
      <div class="panel-copy"><span>Interactive recommendation</span><h4>{{ currentRecommendation }}</h4><p>Expected outcome: {{ expectedOutcome }}</p><button @click="emit('usePrompt', currentRecommendation); record('completed')">Adjust in command canvas</button></div>
