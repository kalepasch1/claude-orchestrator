...
const layers = [
  ...
  { number: '02', name: 'Direction, not prompts', detail: 'State the outcome in your own words. Madeus asks what it still needs to know, then holds that objective through every handoff.' },
  { number: '03', name: 'Best-capability routing', detail: 'Models, tools, specialists, and people are selected by quality, privacy, speed, reliability, and cost.' },
  { number: '04', name: 'Judgment in the loop', detail: 'Legal, compliance, and risk review travel with the work — as it is framed, while it is built, and before it is released.' },
  { number: '05', name: 'Execution and proof', detail: 'Work is tested independently, governed by policy, released on your sign-off, and returned with durable evidence.' },
]

const innovations = [
  { index: '01', name: 'Trojun', label: 'Compounding delivery', title: 'Every release makes the next one sharper.', body: 'Trojun finds the constraint, selects the smallest high-leverage improvement, and independently proves the result — then writes what it learned back into how the next release is built.', glyph: '✦', tags: ['Route', 'Build', 'Verify', 'Learn'] },
  { index: '02', name: 'Portfolio network', label: 'Shared intelligence', title: 'One company learns. Every company improves.', body: 'Proven experience becomes reusable guidance across the portfolio — shared only with explicit permission, separated from where it came from, and never before it has held up more than once.', glyph: '◎', tags: ['Private precedent', 'Explicit consent', 'Proven context'] },
...
const controls = [
  { index: '01', name: 'Steer in your own words', detail: 'Redirect, clarify, or stop the work at any point. Direction from legal, finance, growth, or engineering is recorded against the work and shapes what happens next.' },
  { index: '02', name: 'Sign off before release', detail: 'Work is staged and reviewed against the objective it was given. Release is a decision you make — never a default the system takes for you.' },
  { index: '03', name: 'Share proof, not access', detail: 'Send a reviewer a scoped, read-only evidence link carrying a tamper-evident digest — with no internal access, no credentials, and no raw records attached.' },
  { index: '04', name: 'See what is next', detail: 'Every company, what is in motion, what is scheduled, and what is waiting on you — ordered by what genuinely needs a decision today.' },
]
...
// ---------------------------------------------------------------------------
// SEO. This component is the ONLY publicly rendered surface on www.madeus.cc —
// every application route falls back to it until a member is admitted — so the
// canonical for the public shopfront always points at the site root.
// ---------------------------------------------------------------------------
const SITE = 'https://www.madeus.cc'
const TITLE = 'Madeus — The private operating system for company building'
const DESCRIPTION =
  'Madeus is the private operating system for founders running multiple companies. Give direction once: private context, governed execution, judgment in the loop, and independently verified outcomes. Access is by invitation.'
const OG_IMAGE = `${SITE}/og.png`
const OG_IMAGE_ALT = 'Madeus — Company building, without limits.'
  ...
  '@graph': [
    {
      ...
      url: `${SITE}/`,
      description: DESCRIPTION,
      slogan: 'Company building, without limits.',
      image: OG_IMAGE,
      logo: { '@type': 'ImageObject', url: `${SITE}/madeus-mark.svg` },
    },
    {
      '@type': 'WebSite',
      '@id': `${SITE}/#website`,
...
const softwareLd = {
  ...
  applicationSubCategory: 'Company operating system',
  operatingSystem: 'Web',
  description: DESCRIPTION,
  image: OG_IMAGE,
  inLanguage: 'en',
  ...
  featureList: [
    ...
    'Best-capability routing',
    'Legal and compliance review in the loop',
    'Operator sign-off before release',
    'Independent verification and durable proof',
  ...
  <main class="landing" data-release-surface="public-command-center" data-release-marker="operator-improvements-2026-08-07">
    ...
    <header class="nav">
      <nav aria-label="Primary navigation">
        <a href="#system">Platform</a><a href="#capabilities">Capabilities</a><a href="#control">Control</a><a href="#security">Security</a>
      </nav>
...
.control{padding:135px 24px;background:var(--white);border-top:1px solid var(--line)}.control-grid{display:grid;grid-template-columns:repeat(2,1fr);max-width:1400px;margin:auto;border-top:1px solid var(--black);border-left:1px solid var(--line)}.control-grid article{position:relative;min-height:250px;display:flex;flex-direction:column;padding:30px;border-right:1px solid var(--line);border-bottom:1px solid var(--line);transition:background .3s ease}.control-grid article:hover{background:var(--paper)}.control-grid span{font:9px ui-monospace,monospace;color:var(--green)}.control-grid h3{margin:26px 0 0;font-size:clamp(24px,2.3vw,33px);font-weight:450;letter-spacing:-.04em;line-height:1.05}.control-grid p{max-width:520px;margin:14px 0 0;color:var(--muted);font-size:12px;line-height:1.75}.control-grid i{width:0;height:1px;margin-top:auto;background:var(--green);transition:width .45s cubic-bezier(.2,.7,.3,1)}.control-grid article:hover i{width:64px}.control-note{max-width:1400px;margin:34px auto 0;color:var(--muted);font-size:15px;line-height:1.65;text-wrap:balance}
.landing{scroll-behavior:smooth}.landing :focus-visible{outline:2px solid var(--green);outline-offset:3px}.hero :focus-visible,.innovations :focus-visible,.footer :focus-visible{outline-color:#9db9a6}.section-heading h2,.hero h1,.closing h2,.outcome-copy h2,.security h2{text-wrap:balance}.nav nav a{position:relative;padding-bottom:3px}.nav nav a:after{position:absolute;left:0;bottom:0;width:0;height:1px;background:currentColor;transition:width .35s cubic-bezier(.2,.7,.3,1);content:""}.nav nav a:hover:after{width:100%}.monolith:after{position:absolute;inset:0;background:linear-gradient(115deg,#fff0 38%,#ffffff2e 50%,#fff0 62%);animation:sweep 8s ease-in-out infinite;content:""}.hero-bottom button,.system-footer button,.function-copy button,.security button,.closing button{transition:transform .3s cubic-bezier(.2,.7,.3,1),box-shadow .3s ease}.hero-bottom button:hover,.system-footer button:hover,.function-copy button:hover,.security button:hover,.closing button:hover{transform:translateY(-2px);box-shadow:0 10px 26px #0b0c0b26}.function-grid article{transition:background .3s ease}.function-grid article:hover{background:var(--paper)}.function-grid i{transition:transform .3s cubic-bezier(.2,.7,.3,1)}.function-grid article:hover i{transform:translate(3px,-3px)}.innovation-grid article{transition:background .35s ease}.innovation-grid article:hover{background:#ffffff07}.trust-strip span{transition:color .3s ease}.trust-strip span:hover{color:var(--black)}
@keyframes orbit{to{transform:translate(-50%,-50%) rotate(360deg)}}@keyframes float{50%{transform:translateY(-12px)}}@keyframes pulse{50%{opacity:.3;transform:scale(1.1)}}@keyframes sweep{0%,68%,100%{opacity:0;transform:translateX(-120%)}18%{opacity:1}45%{opacity:0;transform:translateX(120%)}}
