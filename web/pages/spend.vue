  ...
  <div class="min-h-screen bg-white text-gray-900">
    <div class="max-w-6xl mx-auto px-6 py-6 space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-xl font-bold text-gray-900">Spend &amp; ROI</h1>
          <p class="text-sm text-gray-500 mt-0.5">AI cost breakdown, budgets, and return on investment</p>
        </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div class="bg-gray-50 border border-gray-200 rounded-xl p-4">
          <div class="text-2xl font-bold text-gray-900 font-mono">${{ cashMtd.toFixed(2) }}</div>
          <div class="text-xs text-gray-500 mt-1">Cash spend MtD</div>
        </div>
        <div class="bg-gray-50 border border-gray-200 rounded-xl p-4">
          <div class="text-2xl font-bold text-blue-600 font-mono">${{ coveredMtd.toFixed(2) }}</div>
          <div class="text-xs text-gray-500 mt-1">Max-covered MtD</div>
        </div>
        <div class="bg-gray-50 border border-gray-200 rounded-xl p-4">
          <div class="text-2xl font-bold text-green-600 font-mono">{{ mergeRate }}%</div>
          <div class="text-xs text-gray-500 mt-1">Merge rate</div>
        </div>
        <div class="bg-gray-50 border border-gray-200 rounded-xl p-4">
          <div class="text-2xl font-bold text-amber-600 font-mono">{{ usdPerMerge ? `$${usdPerMerge}` : '—' }}</div>
          <div class="text-xs text-gray-500 mt-1">$/merge</div>
        </div>
      </div>

      <!-- Burn-down Chart -->
      <div class="bg-gray-50 border border-gray-200 rounded-xl p-5">
        <div class="text-sm font-semibold text-gray-900 mb-3">Cumulative Spend</div>
      ...
      <div class="bg-gray-50 border border-gray-200 rounded-xl overflow-hidden">
        <div class="px-5 py-3 border-b border-gray-200">
          <span class="text-sm font-semibold text-gray-900">Provider Spend MtD</span>
        </div>
        <div v-if="providerSpend.length === 0" class="px-5 py-6 text-gray-400 text-sm">No spend data</div>
        <table v-else class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="s in providerSpend" :key="`${s.provider}-${s.project}`" class="hover:bg-gray-50">
              <td class="px-5 py-2.5 font-mono text-gray-700 text-xs">{{ s.provider }}</td>
              <td class="px-5 py-2.5 text-xs text-gray-500">{{ s.project || '—' }}</td>
              <td class="px-5 py-2.5 text-right font-mono text-xs" :class="isNotional(s.provider) ? 'text-blue-600' : 'text-gray-900'">${{ Number(s.spent || 0).toFixed(4) }}</td>
              <td class="px-5 py-2.5 text-right">
                <span class="text-xs px-1.5 py-0.5 rounded" :class="isNotional(s.provider) ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-500'">
      ...
      <!-- Budget Bars -->
      <div v-if="projects.length > 0" class="bg-gray-50 border border-gray-200 rounded-xl overflow-hidden">
        <div class="px-5 py-3 border-b border-gray-200">
          <span class="text-sm font-semibold text-gray-900">Budget Usage</span>
        </div>
          ...
          <div v-for="p in projects" :key="p.id">
            <div v-if="budgetFor(p.name).cap !== null" class="space-y-1">
              <div class="flex justify-between text-xs">
                ...
                <span class="font-mono text-gray-500">${{ budgetFor(p.name).spent.toFixed(2) }} / ${{ budgetFor(p.name).cap }}</span>
              </div>
      ...
      <div class="bg-gray-50 border border-gray-200 rounded-xl overflow-hidden">
        <div class="px-5 py-3 border-b border-gray-200">
          <span class="text-sm font-semibold text-gray-900">ROI by Project</span>
        </div>
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="r in roiByProject" :key="r.project" class="hover:bg-gray-50">
              <td class="px-5 py-2.5 text-gray-700">{{ r.project }}</td>
              <td class="px-5 py-2.5 text-right font-mono text-xs text-gray-500">${{ r.spend }}</td>
              <td class="px-5 py-2.5 text-right font-mono text-xs text-green-600">{{ r.merged }}</td>
              <td class="px-5 py-2.5 text-right font-mono text-xs text-amber-600">{{ r.costPerMerge ? `$${r.costPerMerge}` : '—' }}</td>
              <td class="px-5 py-2.5 text-right text-xs" :class="r.passRate >= 70 ? 'text-green-600' : r.passRate >= 40 ? 'text-amber-600' : 'text-red-600'">{{ r.passRate }}%</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="bg-gray-50 border border-gray-200 rounded-xl overflow-hidden">
        <div class="px-5 py-3 border-b border-gray-200">
          <span class="text-sm font-semibold text-gray-900">Spend by Model</span>
        </div>
        <div class="p-5 space-y-3">
          <div v-for="[model, spend] in byModel" :key="model" class="flex items-center gap-3">
            <span class="text-xs font-mono text-gray-500 w-48 truncate">{{ model }}</span>
            <div class="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
              <div class="h-full bg-blue-500 rounded-full" :style="`width: ${byModel.length ? (spend / (byModel[0]?.[1] || 1)) * 100 : 0}%`"></div>
            </div>
            <span class="text-xs font-mono text-gray-500 w-16 text-right">${{ spend.toFixed(3) }}</span>
          </div>
        </div>
      </div>
