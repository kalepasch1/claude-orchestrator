...
export function compileEntityJurisdictionStructure(input: any) {
  ...
  }).sort((a: any, b: any) => a.authority_months - b.authority_months)
  const setup = plan.reduce((sum: number, x: any) => sum + (x.route === 'acquire_regulated_entity' ? 750_000 : x.route === 'sponsor_then_local_entity' ? 150_000 : 300_000), 0)
  return { entity_graph: { entities, ownership_edges: entities.slice(1).map((entity: any) => ({ from: 'parent', to: entity.key, control: 'subject_to_local_requirements' })) }, jurisdiction_plan: plan, ownership_controls: ['change_control_preclearance','beneficial_owner_monitoring','fit_and_proper_refresh'], intercompany_agreements: ['services_and_ip','data_processing','cost_allocation','supervision_and_escalation'], staffing_plan: plan.map((x: any) => ({ jurisdiction: x.jurisdiction, roles: ['responsible_officer','compliance_contact'], before_activation: true })), authority_plan: plan, tax_coordination_flags: ['permanent_establishment','transfer_pricing','withholding','indirect_tax'], timeline: { critical_path_months: Math.max(0, ...plan.map((x: any) => x.authority_months)), sequencing: plan.map((x: any) => x.jurisdiction) }, costs: { setup_cents: setup, annual_carry_cents: plan.length * 120_000 }, expected_value_cents: markets.reduce((s: number, x: any) => s + Number(x.expected_value_cents || 0), 0), residual_risks: ['local_counsel_confirmation','tax_advice_required','regulator_discretion','counterparty_acceptance'], execution_requires_separate_approvals: true }
}
