...
describe('regulatory proof market', () => {
  ...
  it('selects an eligible counterparty without performing an introduction', () => {
    ...
    // recommended_match is nullable (no eligible counterparty is a valid outcome), so assert
    // it is present before reading through it rather than letting TS18047 stand.
    expect(result.recommended_match).not.toBeNull()
