...
const V1 = [
  ...
  ['Orchestrators', '/orchestrators'], ['Connections', '/connectors'],
...
describe('navigation contract v1', () => {
  it('keeps every learned destination in its original position', () => expect(CANONICAL_NAVIGATION.map(item => [item.label, item.to])).toEqual(V1))
  ...
  it('increments only through an explicit contract version', () => expect(NAVIGATION_CONTRACT_VERSION).toBe(1))
})
