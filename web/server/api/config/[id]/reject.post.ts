// POST /api/config/:id/reject — reject a pending config change request.
...
export default defineEventHandler(async (event) => {
  ...
  const [{ error: updErr }, { data: approval, error: insErr }] = await Promise.all([
    sb.from('config_requests').update({ status: 'rejected' }).eq('id', id),
    sb.from('config_approvals').insert({ request_id: id, approver, decision: 'rejected', reason, decided_at: now }).select('*').single(),
  ]);
