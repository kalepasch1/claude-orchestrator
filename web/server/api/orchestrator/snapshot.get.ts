...
export default defineEventHandler(async (_event) => {
  ...
  const [stateResults, runningResult, recentDoneResult, costResult] = await Promise.all([
    ...
    sb.from('tasks')
      ...
      .limit(20),
    // Recently completed. These states are intentionally distinct: DONE is a
    // produced artifact, MERGED is integration-branch reachability, and only
    // DEPLOYED_AND_VERIFIED is production delivery.
    sb.from('tasks')
