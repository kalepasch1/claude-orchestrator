...
export default defineEventHandler(async (event) => {
  ...
  const [sourcesRes, postureRes, findingsRes, briefsRes, memosRes, closeoutsRes] = await Promise.all([
    ...
    // steering PR outcomes power the "fixes in flight" card; the table exists since 2026-09-15 — a
    // 42P01 (absent on an older control plane) must not 500 the whole page.
    scoped(sb.from('db_pr_closeouts').select('project,state,merged,measured_at').order('measured_at', { ascending: false }).limit(200)),
  ])
  ...
  const posturePrev: Record<string, any> = {}
  for (const snap of postureRes.data || []) {
    if (!posture[snap.source_id]) posture[snap.source_id] = snap
    else if (!posturePrev[snap.source_id]) posturePrev[snap.source_id] = snap
  }
  const closeouts = { open_prs: 0, verified: 0, not_confirmed: 0 }
  if (!(closeoutsRes as any).error) for (const c of (closeoutsRes.data as any[]) || []) {
    if (c.state === 'verified-resolved') closeouts.verified++
    else if (c.state === 'merged-not-confirmed') closeouts.not_confirmed++
    else closeouts.open_prs++
  } else console.warn('[db-steering] db_pr_closeouts missing — apply migration 20260915000000:', (closeoutsRes as any).error.message)
  ...
  return { ok: true, days, project: project || null, sources: (sourcesRes.data || []).map(maskSource), posture, posturePrev, closeouts, findings: { open_by_severity, total_open: (findingsRes.data || []).length, top }, briefs: briefsRes.data || [], memos: memosRes.data || [] }
})
